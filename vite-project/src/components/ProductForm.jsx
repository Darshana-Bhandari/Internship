...
const ProductForm = ({ addProduct, editingProduct, updateProducts }) => {
  ...
  useEffect(() => {
    if (editingProduct) {
      ...
      setPrice(editingProduct.price != null ? String(editingProduct.price) : "");
    } else {
      setName("");
      setPrice("");
    }
  ...
  const handleSubmit = (e) => {
    ...
    if (editingProduct && editingProduct.id != null) {
      updateProducts(editingProduct.id, product);
    } else {
      addProduct(product);
    }
  ...
  return (
    <form onSubmit={handleSubmit} className="px-4 mt-6">
      <div className="space-y-4 max-w-sm mx-auto">
        <div>
          <label htmlFor="name" className="mb-2 text-slate-900 font-medium text-sm inline-block">
            Product name
          </label>
          <input
            type="text"
        ...
        <div className="relative">
          <label htmlFor="price" className="mb-2 text-slate-900 font-medium text-sm inline-block">
            Product price
          </label>
          <button
            type="button"
          ...
          >
            <svg xmlns="http://www.w3.org/2000/svg" className="fill-slate-400 overflow-visible" viewBox="0 0 128 128">
              <path
